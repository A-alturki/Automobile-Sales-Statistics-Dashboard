...
def update_output_container(selected_statistics, input_year):
    if selected_statistics == 'Recession Period Statistics':
        ...
        R_chart3 = dcc.Graph(
            figure=px.pie(exp_rec, values="Advertising_Expenditure", names="Vehicle_Type", title="pie graph title idk")
        )

            # Plot 4 bar chart for the effect of unemployment rate on vehicle type and sales
        rec2 = recession_data.groupby("Vehicle_Type")['Automobile_Sales'].mean().reset_index()
        R_chart4 = dcc.Graph(
            figure=px.bar(rec2,x="Vehicle_Type" ,y= "Automobile_Sales", title=" bar graph for  unemployment rate on vehicle type and sales")
        )
    ...
    # Yearly Statistic Report Plots
        ...
        b = yearly_data.groupby('Vehicle_Type')['Automobile_Sales'].mean().reset_index()
        Y_chart3 = dcc.Graph(figure=px.bar(b, x="Vehicle_Type", y="Automobile_Sales"))

        # Total Advertisement Expenditure for each vehicle using pie chart
        exp_data = yearly_data.groupby('Vehicle_Type')['Advertising_Expenditure'].mean().reset_index()
        Y_chart4 = dcc.Graph(figure=px.pie(exp_data, values="Advertising_Expenditure", names="Vehicle_Type", title="pie graph title idk1122"))
